...
class GoodDragger:
    def __init__(
        ...
        self.is_sdxl = False
        self.model = diffusers_pipeline
        self.model.scheduler = scheduler

        if type(diffusers_pipeline) == StableDiffusionXLPipeline:
            self.is_sdxl = True
    ...
    def inv_step(self, model_output: torch.FloatTensor, timestep: int, x: torch.FloatTensor, verbose=False):
        """
        ...
        alpha_prod_t = (
            self.model.scheduler.alphas_cumprod[timestep] if timestep >= 0 else self.model.scheduler.final_alpha_cumprod
        )
    ...
    def image2latent(self, image):
        ...
        latents = self.model.vae.encode(image.to(self.model.device,self.model.dtype))["latent_dist"].mean
    ...
    def get_text_embeddings(self, prompt):
        text_input = self.model.tokenizer(prompt, padding="max_length", max_length=77, return_tensors="pt")
    ...
    def cal_motion_supervision_loss(
        self,
        handle_points,
        target_points,
        F1,
        ...
        for i_ in range(len(handle_points)):
            ...
            f0_patch = original_features[
                :,
                :,
                int(pi[0]) - self.r_1 : int(pi[0]) + self.r_1 + 1,
                int(pi[1]) - self.r_1 : int(pi[1]) + self.r_1 + 1,
            ].detach()
    ...
    def track_step(self, original_feature, original_feature_, F1, F1_, handle_points, handle_points_init):
        if self.compare_mode:
            handle_points = point_tracking(original_feature, F1, handle_points, handle_points_init, self.r_2)
        else:
            handle_points = point_tracking(original_feature_, F1_, handle_points, handle_points_init, self.r_2)
    ...
    def gooddrag_step(
        ...
        drag_latents.requires_grad = True
        ...
        while track_num < self.track_per_denoise:
            ...
            if first_drag:
                ...
                if self.compare_mode:
                    handle_points = point_tracking(
                        features[t.item()].cuda(), F1, handle_points, handle_points_init, self.r_2
                    )
                else:
                    handle_points = point_tracking(
                        features[t_.item()].cuda(), F1_, handle_points, handle_points_init, self.r_2
                    )
            ...
            loss, drag_loss = self.cal_motion_supervision_loss(
                handle_points,
                target_points,
                F1,
                x_prev_updated,
            ...
            need_track = (
                (cur_drag_per_track == self.max_drag_per_track)
                or (drag_loss <= self.drag_loss_threshold)
                or self.once_drag
            )
            if need_track:
                track_num += 1
    ...
    def set_latent_masactrl(self):
        editor = MutualSelfAttentionControl(
            start_step=0, start_layer=10, total_steps=self.n_inference_step, guidance_scale=self.guidance_scale
        )
        if self.lora_path == "":
    ...
    def get_intermediate_images(
        self,
        intermediate_images,
        intermediate_images_original,
        intermediate_images_t_idx,
        valid_timestep,
        text_embeddings,
    ):
        for i in range(len(intermediate_images) - 1):
            current_original_code = intermediate_images_original[i].to(self.model.device)
            current_init_code = intermediate_images[i].to(self.model.device)

            self.set_latent_masactrl()

            for inter_i, inter_t in enumerate(valid_timestep[intermediate_images_t_idx[i] + 1 :]):
                with torch.no_grad():
                    noise_pred_all = self.model.unet(
                        torch.cat([current_original_code, current_init_code]),
                        inter_t,
                    ...
                    current_init_code = self.model.scheduler.step(
                        noise_pred, inter_t, current_init_code, return_dict=False
                    )[0]
                    current_original_code = self.model.scheduler.step(
                        noise_pred_original, inter_t, current_original_code, return_dict=False
                    )[0]
    ...
    def good_drag(
        self, source_image, points, mask, return_intermediate_images=False, return_intermediate_features=False
    ):
        ...
        for i, t in enumerate(tqdm(valid_timestep, desc="Drag and Denoise")):
            if i < self.t2 and self.do_drag and (self.no_change_track_num != self.max_no_change_track_num):
                ...
                init_code, handle_points = self.gooddrag_step(
                    init_code,
                    t,
                    t_,
                    text_embeddings,
            ...
            with torch.no_grad():
                noise_pred_all = self.model.unet(
                    torch.cat([original_init, init_code]),
                    t,
                    encoder_hidden_states=torch.cat([text_embeddings, text_embeddings]),
                )
                noise_pred = noise_pred_all[1]
        ...
        if return_intermediate_images:
            intermediate_images = self.get_intermediate_images(
                intermediate_images,
                intermediate_images_original,
                intermediate_images_t_idx,
                valid_timestep,
