...
from .types import Combo, ComfyWidgetType
...
def register_node(category=None, version=0, display_name=None, output=False):
    def decorator(f):
        ...
        if inspect.isfunction(f):
            ...
            raise Exception(f"Unknow object {f.__name__} for register_node.")

        NODE_CLASS_MAPPINGS[unique_name] = node_class
        if display_name is not None:
            NODE_DISPLAY_NAME_MAPPINGS[unique_name] = display_name
        else:
            NODE_DISPLAY_NAME_MAPPINGS[unique_name] = " ".join(re.split("(?<=[a-z0-9])(?=[A-Z])", f.__name__))
        return f
    return decorator
