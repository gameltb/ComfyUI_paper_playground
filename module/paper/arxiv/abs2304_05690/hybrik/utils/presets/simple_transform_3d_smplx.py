...
from ..transforms import (
    addDPG,
    affine_transform,
    batch_rodrigues_numpy,
    flip_cam_xyz_joints_3d,
    flip_joints_3d,
    flip_thetas,
    flip_twist,
...
class SimpleTransform3DSMPLX(object):
    ...
    def __init__(
        ...
        if self.update_beta:
            ...
            self.smplx_layers_dict = {
                "male": smplx_layer_male_kid,
                "female": smplx_layer_female_kid,
                "neutral": smplx_layer_neutral_kid,
            }
            ...
            for gender, layer in self.smplx_layers_dict.items():
    ...
    def __call__(self, src, label):
        ...
        if self._train and (
            (np.sum(joint_vis[:24, 0]) > self.num_joints_half_body) and (np.random.rand() < self.prob_half_body)
        ):
            self.num_joints = 71
            c_half_body, s_half_body = self.half_body_transform(gt_joints[:, :, 0], joint_vis)
        ...
        if self._train and self._occlusion and False:
            ...
            while True:
                ...
                if (
                    ...
                    synth_w = int(synth_w)
                    synth_h = int(synth_h)
                    src[synth_ymin : synth_ymin + synth_h, synth_xmin : synth_xmin + synth_w, :] = (
        ...
        joint_root = (
            label["root_cam"].astype(np.float32) if "root_cam" in label.keys() else np.zeros((3)).astype(np.float32)
        )
        depth_factor = (
        ...
        if is_kid and gender == "female":
            gendered_smplx_layer = self.smplx_layers_dict["neutral"]
        elif gender == "male":
            gendered_smplx_layer = self.smplx_layers_dict["male"]
        elif gender == "female":
            gendered_smplx_layer = self.smplx_layers_dict["female"]
        else:
            gendered_smplx_layer = self.smplx_layers_dict["neutral"]
        ...
        target_xyz, target_weight_xyz = self._integral_xyz_target_generator(joint_xyz, joint_xyz_vis)
        # print(joints_hybrik)
        ...
        if self.focal_length > 0:
            ...
            cam_scale, cam_trans, cam_valid, cam_error, _ = self.calc_cam_scale_trans2(
                target_xyz.reshape(-1, 3)[:55].copy(), target_uvd.reshape(-1, 3)[:55].copy(), xyz_uvd_weight.copy()
            )
        else:
            assert 1 == 2
            cam_scale = 1
    ...
    def correct_gendered_beta(self, beta_gender, expression_gender, gender_list):
        ...
        expression_v_neutral = torch.einsum(
            "bl,mkl->bmk", [expression_gender, self.shapedirs_dict["neutral"][:, :, 11:]]
        )
        kid_v_neutral = torch.einsum("bl,mkl->bmk", [beta_last, self.kid_shapedir_dict["neutral"]])

        v_shaped_neutral_residual = (
            v_shaped_gender - self.v_template_dict["neutral"] - expression_v_neutral - kid_v_neutral
        )

        betas_regressed = (
            torch.einsum("bmk,mkl->bl", [v_shaped_neutral_residual, self.normed_shapedirs_dict["neutral"]])
        ...
        original_neutral_v = (
            torch.einsum("bl,mkl->bmk", [shape_components, self.shapedirs_dict["neutral"]])
            + self.v_template_dict["neutral"]
        )
        generated_neutral_v = (
            torch.einsum("bl,mkl->bmk", [shape_components_new, self.shapedirs_dict["neutral"]])
            + self.v_template_dict["neutral"]
        )
